...
// Bare brand/use-case tokens passed through `category` expand to the whole
// family instead of doing a literal exact-match against a sub-series name.
// e.g. category="Zenbook" -> every Zenbook-line laptop (Duo/S/A/base, 7
// items), but category="Zenbook S" still means exactly that sub-series (2
// items) since "zenbook s" isn't itself a brand_line/use_case token.
// This is what makes generic queries like "show me zenbook laptops" or
// "show me gaming laptops" return the full family instead of only the 1-2
// items that happen to have that literal string as their category.
const KNOWN_BRAND_LINES = ['zenbook', 'rog', 'tuf', 'vivobook', 'proart'];
const KNOWN_USE_CASES = ['gaming', 'creator', 'student', 'productivity', 'business', 'travel', 'budget', 'professional'];
 ...
 * @param {string} [filters.category] Exact series/category match, OR a bare brand_line/use_case token (see KNOWN_BRAND_LINES/KNOWN_USE_CASES above)
...
function searchProducts(catalog, filters = {}) {
  ...
  // Expand a bare brand/use-case token passed through `category` before
  // doing anything else, so both search-products and browse-products-by-series
  // get "whole family" behavior for generic queries.
  let effectiveCategory = typeof category === 'string' ? category.trim() : '';
  let effectiveBrandLine = brand_line;
  let effectiveUseCase = use_case;
  if (effectiveCategory && !brand_line && !use_case) {
    const catToken = effectiveCategory.toLowerCase();
    if (KNOWN_BRAND_LINES.includes(catToken)) {
      effectiveBrandLine = catToken;
      effectiveCategory = '';
    } else if (KNOWN_USE_CASES.includes(catToken)) {
      effectiveUseCase = catToken;
      effectiveCategory = '';
    }
  }
  ...
  const line = typeof effectiveBrandLine === 'string' ? effectiveBrandLine.trim().toLowerCase() : '';
  const useCase = typeof effectiveUseCase === 'string' ? effectiveUseCase.trim().toLowerCase() : '';
  const minGpuTierIdx = gpu_tier ? GPU_TIERS.indexOf(String(gpu_tier).toLowerCase()) : -1;

  const appliedFilters = [];
  if (cat) appliedFilters.push(`series "${effectiveCategory}"`);
  if (line) appliedFilters.push(`brand "${line}"`);
  if (useCase) appliedFilters.push(`use case "${useCase}"`);
  ...
  // Broad "whole family" browsing (bare brand_line/use_case, no free-text
  // query) defaults to showing everything in that family rather than an
  // arbitrary top-5 slice, unless the caller passed an explicit limit.
  const isBroadBrowse = (line || useCase) && !q && filters.limit === undefined;
  const effectiveLimitInput = isBroadBrowse ? 10 : limit;
  const cappedLimit = Math.max(1, Math.min(Number(effectiveLimitInput) || 5, 10));
  const results = scored.slice(0, cappedLimit).map((s) => s.item);

  return { results, appliedFilters };
}

module.exports = { searchProducts, KNOWN_BRAND_LINES, KNOWN_USE_CASES };
